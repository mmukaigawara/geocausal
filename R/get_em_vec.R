...
#' @param dimyx a vector of 2 integer specifying the pixel dimension. If NULL, the pixel dimension of the original
...
get_em_vec <- function(em,time_after = TRUE,lag,entire_window = NULL,dimyx=NULL){
  
  l <- length(em)
  res <- imls_to_arr(em,start=1,end=l-time_after-lag+1,entire_window,dimyx = dimyx)
  res <- c(res)
