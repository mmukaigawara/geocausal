...
dist_from_focus2 <- get_dist_focus(window = iraq_window,
                                   longitude = c(43.158, 44.366), #Mosul and Baghdad
                                   latitude = c(36.349, 33.315),
                                   grayscale = FALSE,
                                   mile = FALSE)
...
```{r}
distance_from_focus <- exp(-dist_from_focus[[1]]/1000)/spatstat.geom::integral(dist_from_focus[[1]])
...
# The process above takes so much time. Perhaps distmap -> convert coordinate system
# Or, convert coordinates of shapefile -> then distmap?
...
shapefile_path = "~/geocausal/Data/covariates/primary_routes/primary_routes.shp"
distance_from_line <- get_dist_line(window = iraq_window, path_to_shapefile = shapefile_path)
