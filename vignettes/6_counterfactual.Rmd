...
sim_power_mosul <- sim_power_density(target_densities = list(dist_from_routes), #This must be a list element
                                          density_to_manipulate = dist_mosul,
                                          priorities = 1,
                                          priorities_for_manipulation = c(1, 2, 5, 10, 15, 50),
                                          window = iraq_window,
                                          grayscale = FALSE)
...
```{r}
# Simulation
sim_counter_mosul <- sim_counterfactual_density(expected_number = target_number,
...
sim_power_road <- sim_power_density(target_densities = list(dist_mosul), #This must be a list element
                                    density_to_manipulate = dist_from_routes,
                                    priorities = 1,
...
```{r}
sim_counter_road <- sim_counterfactual_density(expected_number = target_number,
                                               baseline_density = baseline_density,
                                               power_simulation_results = sim_power_road,
                                               window = iraq_window,
                                               grayscale = FALSE)
...
```{r}
expectation_road <- get_distance_based_expectation(counterfactual_simulation_results = sim_counter_road, 
                                                   entire_window = iraq_window, 
                                                   density_of_interest = dist_from_routes,
                                                   distance_map = dist_from_line$distance_im,
                                                   distance_map_unit = "km",
                                                   grayscale = FALSE,
                                                   expectation_use_raw = TRUE)
