...
min(dat_hfr$time) #The min (day 31) is equivalent to March 25, 2007
max(dat_hfr$time) #The max (day 499) is July 5, 2008

# Pick the date when the actual combat was over
surge_date <- as.numeric(as.Date("2008-01-01") - as.Date("2007-03-25") 
                         + min(dat_hfr$time))

dat_hfr$surge <- ifelse(dat_hfr$time <= surge_date, 1, 0)
...
plot_compare_surge$plot_compare +
  ...
  annotate("text", label = "Predicted\n(surge)", x = 250, y = 70, size = 4, color = "#0072B2") +
  ylim(0, 100)
```
