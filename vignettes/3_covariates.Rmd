...
Our example has the following covariates:
...
The point processes summarize the locations of major cities based on the population size. The pixel images show the distances from major cities.
...
The point processes show the locations of settlements in each Governorate in Iraq. The pixel images show the distances from the settlements. We save each of these after multiplying by some constants and then exponentiating them.
...
```{r, out.height=400, out.width=600, fig.dim = c(9, 6)}
#try <- readRDS("~/geocausal/Data/dat_hfr_3.rds")
#dat_hfr$aid <- try$aid
```
...
rm(aid_district, cities_dist, covs_priority_coefs, 
   iraq3, rivers_dist, routes_dist,
   sectarian, settle_dist, spline_data, 
   time_splines, try)
```