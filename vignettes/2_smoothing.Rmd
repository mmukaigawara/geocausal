---
title: "Step 2. Smoothing outcomes"
output: 
  rmarkdown::html_vignette:
    toc: true
    number_sections: true
vignette: >
  %\VignetteIndexEntry{2_smoothing_outcomes}
...
The purpose of this vignette is letting users smooth the point processes. 

By the end of this vignette, users will understand the usage of <tt>get_smoothed_outcome</tt>.

# Smoothing outcomes

A key step of spatiotemporal causal inference is to transform outcomes (point processes) to smoothed outcomes. To do so, we use <tt>get_smoothed_outcome</tt> function. This function has following arguments:

- <tt>data_interest</tt>: a hyperframe column that you want to convert (in the form of <tt>hyperframe$column</tt>); and
...
To use this method, users first need to set <tt>method = "mclust"</tt> and then specify the <tt>initialization</tt> argument. By setting <tt>initialization = TRUE</tt>, the function uses a small fraction of data to obtain the common variance, which makes the process even faster.
...
#saveRDS(dat_hfr, file = "dat_hfr.RData")
```
